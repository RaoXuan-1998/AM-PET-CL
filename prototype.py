...
def gen_proto(args, feature_extractor, dataset, prompts=None, load_path='', name='', save=True):
    """
    Generate class-wise prototype using class mean vector.
    """
    if load_path:
        prototype_dict = torch.load(load_path)
        prototypes = prototype_dict['means'].cuda()
        prototypes_var = prototype_dict['vars'].cuda()
        prototypes_similarity = prototype_dict['similarity']
        all_features = prototype_dict['features']
    else:
        with torch.no_grad():
            ...
            for class_id in tqdm_gen:
                ...
                gen_single_proto(args, feature_extractor, class_id, dataset, similarity)
...
def gen_single_proto(args, feature_extractor, class_id, dataset, similarity=True):
    with torch.no_grad():
        feature_list = []
