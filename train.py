...
class Trainer(object):
    ...
    def train_discriminate(self):
        ...
        if torch.cuda.device_count() > 1:
            ...
            self.prompter = torch.nn.DataParallel(self.prompter, device_ids=self.args.device_ids)
        ...
        for current_session in range(self.args.task_num + 1):
            ...
            # set current prompter
            if self.prompter is not None and self.args.init_with_prev_prompt:
                prev_prompts = self.prompter.module.prompts.detach() if \
                    isinstance(self.prompter, torch.nn.DataParallel) else \
                          self.prompter.prompts.detach()
            else:
                prev_prompts = None

            self.prompter = ProTLearner(self.args, self.vit_model, prev_prompts)
            
            # Check if multi-GPU setup and wrap the model accordingly
            if torch.cuda.device_count() > 1:
                self.prompter = torch.nn.DataParallel(self.prompter, device_ids=self.args.device_ids)
            
            self.prompter = self.prompter.cuda()
            prompt_param = self.prompter.module.get_prompt_param() if \
                isinstance(self.prompter, torch.nn.DataParallel) else self.prompter.get_prompt_param()
            ...
            self.head = PromptHead(self.args, self.prompter.module.vit_model.embed_dim).cuda() if \
                  isinstance(self.prompter, torch.nn.DataParallel) else PromptHead(self.args, self.prompter.vit_model.embed_dim).cuda()
            ...
            for proto_id in classes:
                self.prototype_prompts.append(copy.deepcopy(self.prompter.module.prompts) if \
                    isinstance(self.prompter, torch.nn.DataParallel) else \
                        copy.deepcopy(self.prompter.prompts)) 
